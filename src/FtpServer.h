...
private:
	...
	bool makeDir(const char *path)
	{
		return STORAGE_MANAGER.mkdir(path);
	};
...
//  bool openFile( char path[ FTP_CWD_SIZE ], int readTypeInt );
	...
	uint32_t capacity()
	{
		return STORAGE_MANAGER.card()->cardSize() >> 1;
	};
	uint32_t free() { return STORAGE_MANAGER.vol()->freeClusterCount() *
								 STORAGE_MANAGER.vol()->sectorsPerCluster() >>
							 1; };
#elif STORAGE_TYPE == STORAGE_SDFAT2
	uint32_t capacity()
	{
		return STORAGE_MANAGER.card()->sectorCount() >> 1;
	};
	uint32_t free() { return STORAGE_MANAGER.vol()->freeClusterCount() *
								 STORAGE_MANAGER.vol()->sectorsPerCluster() >>
							 1; };
	...
	uint32_t capacity()
	{
		return STORAGE_MANAGER.capacity();
	};
